...
from mindspore import context, nn, Tensor, set_seed, Model, SummaryCollector, load_checkpoint, load_param_into_net
from mindspore.context import ParallelMode
from mindspore.communication.management import init, get_rank, get_group_size
from src.config import config
from src.dataset import create_dataset_train
from src.lr_generator import get_lr
from src.network import NTS_NET, WithLossCell
...
parser.add_argument('--data_url', default='./dataset',
                    help='Directory contains resnet50.ckpt and CUB_200_2011 dataset.')
parser.add_argument('--train_url', default='./cache',
...
if __name__ == '__main__':
    ...
    # loss_cb = ms.LossMonitor(per_print_times=dataset_size)
    loss_cb = ms.LossMonitor(300)
    ...
    if config.save_checkpoint and rank == 0:
        ...
        # ckpoint_cb = ms.ModelCheckpoint(prefix=config.prefix, directory=save_checkpoint_path, ckconfig=ckptconfig,
        #                              device_num=device_num, device_id=device_id, args=args, run_modelart=run_modelart)
        ckpoint_cb = ms.train.ModelCheckpoint(prefix=config.prefix, directory=save_checkpoint_path, config=ckptconfig)
        cb += [ckpoint_cb]

        specified = {"collect_metric": True, "histogram_regular": "^conv1.*|^conv2.*", "collect_graph": True,
                     "collect_dataset_graph": True}
        save_result = os.path.join(local_output_url, "summary_dir" + "/")
        summary_collector = SummaryCollector(summary_dir=save_result, collect_freq=300, keep_default_action=False,
                                             collect_specified_data=specified, collect_tensor_freq=200)
